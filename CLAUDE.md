...
- 写入前重新读取目标文件, 不依赖缓存.
- 若目标文件在读取后发生变化, 基于最新内容重新生成修改.
- 仅对"我即将写入的目标文件"做冲突检查, 防止覆盖用户改动.
- 非目标文件的新增或修改默认忽略, 不中断流程, 不主动询问.
- 仅当目标文件存在并发修改, 或用户要求纳入相关变更时, 才暂停并确认.
- 未经允许不覆盖或回滚用户未提交的本地改动.
- 文档中的引用链接默认使用当前文件的相对路径(例如 `./xx.md`, `../module/xx.md`), 以保证 IDE/GitHub 可直接跳转.
- 如需在正文中展示仓库路径, 用纯文本或代码样式标注(例如 `doc/xxx.md`), 不作为 Markdown 链接目标.
