...
- 密码场景只需要"校验是否一致", 不需要还原明文, 所以应使用单向哈希, 而不是 AES 这类可逆加密.
- Argon2id 是 memory-hard 算法, 不仅消耗 CPU, 还消耗内存, 能显著提高 GPU/ASIC 暴力破解成本.
- 每次哈希都使用随机盐, 同一密码不会得到相同哈希, 可抵御彩虹表和批量撞库.
- 参数可调(`memoryKiB`/`iterations`/`parallelism`), 随着硬件升级可以逐步提高成本, 无需一次性重构.
- PHC 字符串自带版本、参数、盐和哈希, 便于落库、校验和参数迁移.
