...
import com.zhengshuyun.lava.pay.alipay.exception.AlipayPayApiException;
 ...
 */
public final class AlipayPayResponseParser {
     ...
     */
    public AlipayPayResponseParser(PublicKey alipayPublicKey, JsonCodec jsonCodec) {
     ...
     * @return 已验签业务响应
     */
    public <T> T parse(String method, byte[] body, Class<T> responseType,
                       @Nullable String traceId) {
        if (body.length == 0) {
            throw new AlipayPayProtocolException("支付宝响应缺少正文");
        }
        ...
        try {
            ...
            throw new AlipayPayProtocolException("支付宝响应不是有效 JSON");
        }
        ...
        if ((responseSource == null) == (errorSource == null)) {
            throw new AlipayPayProtocolException("支付宝响应缺少唯一业务节点");
        }
        ...
        boolean valid = AlipayPayCryptoUtils.verify(source, signature, alipayPublicKey);
        if (!valid && source.contains("\\/")) {
            // 与支付宝官方 Java SDK 保持兼容：部分网关会对斜杠转义形式做等价签名。
            valid = AlipayPayCryptoUtils.verify(
        ...
        try {
            ...
            throw new AlipayPayProtocolException("支付宝业务响应结构无效");
        }
        String code = AlipayPayValidationUtils.requireResponseText(status.code, "code");
        String message = AlipayPayValidationUtils.requireResponseText(status.message, "msg");
        if (!"10000".equals(code)) {
            throw new AlipayPayApiException(code, message, status.subCode,
                    status.subMessage, traceId);
        }
        if (responseSource == null) {
            throw new AlipayPayProtocolException("支付宝成功码位于错误响应节点");
        }
        try {
            return jsonCodec.read(source, responseType);
        } catch (JsonException exception) {
            throw new AlipayPayProtocolException("支付宝业务响应不是预期结构");
        }
    }
    ...
    private Map<String, String> topLevelValues(String json) {
        ...
        if (index >= json.length() || json.charAt(index) != '{') {
            throw new AlipayPayProtocolException("支付宝响应根节点必须是 JSON 对象");
        }
        ...
        while (index < json.length()) {
            if (json.charAt(index) != '"') {
                throw new AlipayPayProtocolException("支付宝响应属性名无效");
            }
            ...
            if (index >= json.length() || json.charAt(index) != ':') {
                throw new AlipayPayProtocolException("支付宝响应属性缺少分隔符");
            }
            ...
            if (result.putIfAbsent(name, json.substring(valueStart, valueEnd)) != null) {
                throw new AlipayPayProtocolException("支付宝响应包含重复顶层属性");
            }

            index = skipWhitespace(json, valueEnd);
            if (index >= json.length()) {
                throw new AlipayPayProtocolException("支付宝响应 JSON 未闭合");
            }
            char separator = json.charAt(index++);
            if (separator == '}') {
                if (skipWhitespace(json, index) != json.length()) {
                    throw new AlipayPayProtocolException("支付宝响应 JSON 包含尾随内容");
                }
                return result;
            }
            if (separator != ',') {
                throw new AlipayPayProtocolException("支付宝响应属性分隔符无效");
            }
    ...
    private static int valueEnd(String json, int start) {
        if (start >= json.length()) {
            throw new AlipayPayProtocolException("支付宝响应属性缺少值");
        }
        ...
        for (int index = start; index < json.length(); index++) {
            ...
            if (current == '"' && escapes % 2 == 0) {
                ...
                if (depth == 0 || current == '}' && stack[depth - 1] != '{'
                    ...
                    throw new AlipayPayProtocolException("支付宝响应 JSON 嵌套结构无效");
                }
                if (--depth == 0) {
                    return index + 1;
                }
        ...
        throw new AlipayPayProtocolException("支付宝响应 JSON 嵌套结构未闭合");
    }
    ...
    private static int stringEnd(String json, int start) {
        ...
        throw new AlipayPayProtocolException("支付宝响应 JSON 字符串未闭合");
    }
